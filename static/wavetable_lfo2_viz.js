export function initWavetableLfo2Viz() {
  const canvas = document.getElementById('lfo2-canvas');
  if (!canvas) return;
  ...
  const rateEl = qRange('Voice_Modulators_Lfo2_Time_Rate');
  const syncRateEl = qRange('Voice_Modulators_Lfo2_Time_SyncedRate');
  const syncSel = qSelect('Voice_Modulators_Lfo2_Time_Sync');
  const shapeSel = qSelect('Voice_Modulators_Lfo2_Shape_Type');
  const amountEl = qRange('Voice_Modulators_Lfo2_Shape_Amount');
  const attackEl = qRange('Voice_Modulators_Lfo2_Time_AttackTime');
  const offsetEl = qRange('Voice_Modulators_Lfo2_Shape_PhaseOffset');

  const rateItem = document.querySelector('.param-item[data-name="Voice_Modulators_Lfo2_Time_Rate"]');
  const syncRateItem = document.querySelector('.param-item[data-name="Voice_Modulators_Lfo2_Time_SyncedRate"]');
...
document.addEventListener('DOMContentLoaded', initWavetableLfo2Viz);
