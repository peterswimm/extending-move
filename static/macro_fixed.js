document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.param-item').forEach(item => {
    ...
    function updateBase() {
      let cur = null;
      const hid2 = item.querySelector('input[type="hidden"][name$="_value"]');
      if (hid2) {
        const num = parseFloat(hid2.value);
        cur = isNaN(num) ? hid2.value : num;
      } else {
        const sl2 = item.querySelector('.rect-slider');
        if (sl2) cur = parseFloat(sl2.dataset.value || '0');
        else {
          const sel2 = item.querySelector('select.param-select');
          if (sel2) cur = sel2.value;
        }
      }
      if (cur !== null) baseParamValues[name] = cur;
    }
