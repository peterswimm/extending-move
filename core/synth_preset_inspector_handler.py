...
def get_melodic_sampler_sample(preset_path):
    """Return the sample name and path for the first MelodicSampler device."""
    try:
        with open(preset_path, "r") as f:
            data = json.load(f)
        ...
        def search(obj):
            nonlocal sample_path, sample_name
            if isinstance(obj, dict):
                if obj.get("kind") == "melodicSampler":
                    uri = obj.get("deviceData", {}).get("sampleUri")
                    if uri:
                        if uri.startswith("ableton:/user-library/Samples/"):
                            sample_path = uri.replace(
                                "ableton:/user-library/Samples/",
                                "/data/UserData/UserLibrary/Samples/",
                            )
                        else:
                            sample_path = uri.split("file://")[-1]
                        sample_path = unquote(sample_path)
        ...
        return {
            ...
            "sample_path": "",
            "sample_name": "",
            "message": f"Error reading sample info: {exc}",
        }
