...
class SetManagementHandler(BaseHandler):
    ...
    def handle_post(self, form):
        ...
        if action == 'create':
            ...
            if not set_name:
                return self.format_error_response("Missing required parameter: set_name", options=pad_options)
            ...
            return self.format_error_response(f"Unknown action: {action}", options=pad_options)
        ...
        if not pad_selected or not pad_selected.isdigit():
            return self.format_error_response("Invalid pad selection", options=pad_options)
        if not pad_color or not pad_color.isdigit():
            return self.format_error_response("Invalid pad color", options=pad_color_options)
        pad_selected_int = int(pad_selected) - 1
        pad_color_int = int(pad_color)
        # Prepare bundling of generated set
        set_path = result.get('path')
        if not set_path:
            return self.format_error_response("Internal error: missing set path", options=pad_options)
        bundle_path = set_path + '.ablbundle'
        with zipfile.ZipFile(bundle_path, 'w') as zf:
            zf.write(set_path, os.path.basename(set_path))
        # Restore to device
        restore_result = restore_ablbundle(bundle_path, pad_selected_int, pad_color_int)
        os.remove(bundle_path)
        if restore_result.get('success'):
            return self.format_success_response(restore_result['message'])
        else:
            return self.format_error_response(restore_result.get('message'))
